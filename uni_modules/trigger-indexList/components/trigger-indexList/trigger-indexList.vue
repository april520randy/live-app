<template>
	...
	<view class="root" :style="{ height: height || contentHeight }">
		...
		<scroll-view :scroll-top="scrollTop" class="scrollView" :scroll-into-view="currentScrollIntoView"
			@scroll="scroll" scroll-y="true">
		...
		<view class="indexBox" @touchend="touchend" @touchstart="touchstart" @touchmove="touchmove">
			<view class="index" :style="currentIndex == index ? `color:#ED5F2D` : ''" v-for="(item, index) in indexList"
				:key="index">
				{{ item.name }}
			</view>
		</view>
			...
			async initData() {
				...
				this.$nextTick(() => {
					setTimeout(async () => {
						...
						indexBoxDomInfo.childHeight = indexBoxDomInfo.height / this.indexList
							.length

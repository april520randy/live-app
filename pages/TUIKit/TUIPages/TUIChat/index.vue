<template>
	...
	<var-app-bar style="border: none; position: fixed; top:0;z-index:2;background-color: #fff;" color="#fff" round 
		:elevation='false'>
		<template #content>
			<view class='nav'>
				...
				<view class="navtitle">{{conversationName}}</view>
				<!-- <image @click="handleGetProfile" v-if="conversation?.type === 'GROUP'" src="/static/image/set.png"
					style="width: 40rpx;height: 40rpx;position: absolute;right: 20rpx;"/> -->
				<view style="width:80rpx;height: 40rpx;position: absolute;right: 20rpx;color:#000;font-size: 28rpx;"  v-if="conversation?.type === 'GROUP'" @click="handleGetProfile">
					更多</view>
			</view>
		</template>
	</var-app-bar>
	...
	<view class="TUIChat" style="" v-if="conversationType === 'chat'">
			...
			<scroll-view class="TUIChat-main" scroll-y="true" :scroll-with-animation="true"
             ...
             caculateTimeago(item.time * 1000)
						...
						<MessageBubble v-if="!item.isRevoked && item.type !== types.MSG_GRP_TIP" :data="item">
							<Message-Operate v-if="dialogID === item.ID" :data="item" class="dialog dialog-item" :style="{
	...
	export default defineComponent({
		...
		setup(props) {
			...
			onLoad((options) => {
				...
				data.conversationName = options && options.conversationName
			});
			onUnload(() => {
				// #ifdef MP-WEIXIN
				//回收 TUICallKit

<template>
	...
	<view class="manage" ref="dialog">
		...
		<view class="main" v-if="!currentTab">
			...
			<view class="quntitle">
				<image style="margin:0 24rpx;width:100rpx;height:100rpx;border-radius: 50%;"
					:src="
				  conversation?.groupProfile?.avatar ||'https://sdk-web-1252463788.cos.ap-hongkong.myqcloud.com/im/demo/TUIkit/web/img/constomer.svg'"
					onerror="this.src='https://sdk-web-1252463788.cos.ap-hongkong.myqcloud.com/im/demo/TUIkit/web/img/constomer.svg'">
				</image>
				<view>
					<view style="color:#333333;font-size: 30rpx;margin-bottom:20rpx">
						{{ conversation.groupProfile.name }}
					</view>
					<view style="color:#9EA2AF;font-size: 26rpx;">
						群聊号码：{{ conversation.groupProfile.groupID }}
					</view>
				</view>
			</view>

			<view class="userInfo space-top">
				<view class="userInfo-header" style="padding:0 24rpx" @click="setTab('member')">
					<view> 群成员 <text
							style="color:#9EA2AF;">{{ conversation.groupProfile.memberCount}}/{{ conversation.groupProfile.maxMemberCount}}</text>
					</view>
					<view style="display: flex;align-items: center;">
						查看全部
						<image class="image-right" src="../../../assets/icon/right-arrow.svg"></image>
					</view>
				</view>
				<view style="border-bottom: 1rpx solid #E7E9EE;margin-bottom:32rpx">
				</view>
				<view style="display: flex;padding:0 24rpx">
					<view v-for="(item, index) in userInfo?.list?.slice(0, showUserNum)" :key="index"
						style="margin-right:35rpx;text-align: center;">
			...
			<view
				style="height:96rpx;line-height: 96rpx;color: #333333;font-size: 28rpx;border-bottom: 1rpx solid #E7E9EE;padding-left:24rpx">
				群介绍</view>
			<view style="margin-left:24rpx;height:132rpx;color:#8E92A4;margin-top:20rpx">{{conversation.groupProfile.introduction||'暂无介绍'}}</view>
		...
		<ManageMember v-else-if="currentTab === 'member'" :self="conversation.groupProfile.selfInfo"
			:list="userInfo.list" :total="conversation.groupProfile.memberCount?.memberCount"
	...
	const TUIGroupManage = defineComponent({
		...
		setup(props : any, ctx : any) {
			...
			const quit = async (group : any) => {
				...
				data.config.userID=String(uni.getStorageSync("user_Id"))

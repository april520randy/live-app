	...
	<view class="content">
			...
			<template #content>
				<view class="var-app-bar-content">
					<view class="logo-content">
						<image src="../../static/image/login/icon1.png" class="logo"> </image>
		...
		<view class="part part-1">
			...
			<view class="form">
				...
				<view class="pwd-text">
					<image src="../../static/image/login/key.png" class="key"> </image>
					<input class="uni-input" :password="!showPassword" v-model="formData.User_Password" 
						placeholder="6-12位密码" />
					<image src="../../static/image/login/close.png" v-if="!showPassword" @click="showPassword = true"
						class="eye"> </image>

					<image src="../../static/image/login/open.png" class="eye" v-else @click="showPassword = false">
					</image>

				</view>
				<view class="tip" style="display: block;">
					<text>忘记密码</text>
				</view>
				<view class="user-tip">
			...
			<view class="btns">
				...
				<button type="warn" :disabled="disabled" @click="login">登录</button>

			</view>
			<view class="pwd-login" @click="$emit('changeType', 1)">
				验证码登录
			</view>
...
<script>
import { phoneRegular } from '/utils/index'
export default {
	data() {
		return {
			formData: {
	...
	computed: {
		disabled() {
			if (this.formData.User_Phone && this.formData.User_Password) {
				return false
			}
			return true
		},
	...
	methods: {
		...
		login() {
			...
			this.$emit('pwdLogin', this.formData)
